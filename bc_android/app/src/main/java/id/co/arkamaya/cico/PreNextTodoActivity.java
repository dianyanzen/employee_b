...
public class PreNextTodoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pre_next_todo);
    ...
    private void getListNextTodo(){
        ...
        restInterface.getDailyReport_NextTodo_list(EmployeeId, new Callback<List<GetListDailyReport>>() {
            @Override
            public void success(List<GetListDailyReport> m, Response response) {
                list_next_todo.setAdapter(null);
                if (m.size() > 0) {
                    AdapterNextTodoListView adapt = new AdapterNextTodoListView(getApplicationContext().getApplicationContext(), R.layout.list_adapter_next_todo, m);
                    list_next_todo.setAdapter(adapt);
                }
